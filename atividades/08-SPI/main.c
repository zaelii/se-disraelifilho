...
void salvar_temperatura_sdcard(float temp) {
    FILE* f = fopen("/sdcard/teste.txt", "a");  // Arquivo virtual "teste.txt"
    if (f != NULL) {
...
void app_main(void) {
    ...
    while (1) {
        ...
        bool alarme = temperatura_lida >= temperatura_limite;

        if (alarme) ativar_buzzer();
        else desativar_buzzer();

        atualizar_leds(temperatura_lida, temperatura_limite, alarme);
        atualizar_lcd();
        salvar_temperatura_sdcard(temperatura_lida);

        // LE o conteúdo do arquivo após cada gravação
        ler_arquivo_sdcard();

        vTaskDelay(pdMS_TO_TICKS(2000));  // evitar flood no console
    }
}